...
(function(){var start=window.location.pathname.lastIndexOf("/");window.directory=window.location.pathname.substr(start+1);if(window.directory.indexOf("php")==-1){window.directory="index.php";}
var navMenu=$("#nav").find("a");navMenu.each(function(indx,element){var link=$(this).attr("href").replace("#","");if(window.directory.indexOf(link)>=0){$(this).parent().addClass("current");window.directory_name=($(this).attr("parent")==undefined?"":$(this).attr("parent"))+$(this).parent().text();}});})();$(document).ready(function(){var inp=$("[tooltip]");inp.focus(function(){var obj=$(this).next(".tooltip");if(obj.length==0){$(this).after("<div class='tooltip'>"+$(this).attr("tooltip")+"</div>");obj=$(this).next(".tooltip");var h=-40-obj.height()-2*$(this).height();var l=$(this).offset().left;obj.css({marginTop:h+"px",marginLeft:l+"px"}).fadeIn(200);}else{var h=-40-obj.height()-2*$(this).height();var l=$(this).offset().left;obj.css({marginTop:h+"px",marginLeft:l+"px"}).fadeIn(200);}});inp.blur(function(){$(this).next(".tooltip").hide();});$("[type=tel], [type=number]").on('keypress',function(e){var now=$(this).attr("type");if(now=="tel"||now=="number"){var key=(typeof e.charCode=='undefined'?e.keyCode:e.charCode);if(e.ctrlKey||e.altKey||key<32){return true;}
key=String.fromCharCode(key);var res=/\d/.test(key);return res;}});$("[mask]").each(function(){var mask=$(this).attr("mask");$(this).inputmask({"mask":mask});});$(".ul_start").hover(function(){var css={normal:{backgroundColor:"rgba(57, 57, 57, 0.75)",color:"white",borderRadius:""},hover:{backgroundColor:"rgba(157, 157, 157, 0.75)",color:"rgba(0,0,0,0.4)",borderRadius:"0.25em",},textNormal:{textShadow:""},textHover:{textShadow:"1px 1px 6px #bbb, 0px 0px 0px #000, 1px 1px #bbb"}}
var hover=$(this).attr("hover")=="false"?false:true;$(this).attr("hover",!hover).animate((hover?css.normal:css.hover),300).css((hover?css.textNormal:css.textHover));});});
var modal = {
	el : 0,
	outEl : 0,
	close : 0,
...
String.prototype.replaceAll=function(token,newToken,ignoreCase){var _token;var str=this+"";var i=-1;if(typeof token==="string"){if(ignoreCase){_token=token.toLowerCase();while((i=str.toLowerCase().indexOf(token,i>=0?i+newToken.length:0))!==-1){str=str.substring(0,i)+
newToken+
str.substring(i+token.length);}}else{return this.split(token).join(newToken);}}
return str;};fnDelay=(function(){var timer=0;return function(callback,ms){clearTimeout(timer);timer=setTimeout(callback,ms);};})();var Fn=function(){this.timer=0;this.delay=function(callback,ms){clearTimeout(this.timer);this.timer=setTimeout(callback,ms);};return this;}
...
var formsChecker = {
  ...
  changeState: function(type, name, phone){
    ...
    cookie.set("forms-checker-state", this.state, {expires: 30*24*60*60});
    $.ajax({
      ...
      success: function(data){
        ...
        cookie.set("forms-checker-id", _this.userId, {expires: 30*24*60*60});
      }
    });
  },
  getCookies: function(){
